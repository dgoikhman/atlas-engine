...
{% if asm and asm.assumability|int >= 60 %}<p class="quick"><b>Assumable-mortgage pocket:</b> {{ asm.gov_share_pct }}% of {{ m.state }}'s 2019-21 mortgage originations were government-backed (FHA/VA/USDA) — assumable by law, mostly at ~3% rates. VA loans can be assumed by non-veterans, including for investment purchases. <a href="{{ base }}/rentals/assumable-mortgage-markets/">The Assumability Index →</a></p>{% endif %}
